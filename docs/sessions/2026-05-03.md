...
ADR-009 population rule is **landed and DB-verified**. Remaining
threads, in rough priority order:

1. **Idempotent re-ingest** — on `content_hash` match, skip the
   INSERT instead of tripping `uk_legal_documents_mst`. Today's
   re-run on a populated `pgdata` volume errors mid-Phase-1.
   Small surface; probably one SELECT before each INSERT.
2. **`_insert_children` parser depth** — the larger thread.
...
3. **Packaging** — `pyproject.toml` with src layout, pinned deps,
   dev deps (`pytest`, `ruff`). Stops needing `PYTHONPATH=src` on
   the host; lock file replaces `requirements.txt`.
4. **ADR-006 verification trigger** — `parse_doc` currently raises
...
5. **Migration tool selection** — `docker-entrypoint-initdb.d`
