...
1. **ADR-009 population rule implementation** — ingestion-pipeline
   ...
   parent lookup via the head Act title index.
...
flowchart TD
  ...
  Discover --> Parse["Parse XML; extract doc-level fields<br/>(title, doc_type, head/effective timestamps, …)"]
  ...
  StripD --> LookupD["SELECT doc_id FROM legal_documents<br/>WHERE title = stripped<br/>AND doc_type = '법률' AND is_head = TRUE<br/>(uses head Act title index)"]
  LookupD --> Match{match?}
...
- Title-strip + parent lookup via
  head Act title index (ADR-009 patch #2, amended by ADR-013)
...
3. **Amendment-tracking ADR, later drafted as ADR-013** — the policy
   deciding what `_skip_if_present`'s mismatch branch should do
   for real amendments. Today: hard fail. Future: auto-supersede
   with `is_head=FALSE` + `superseded_at=incoming_effective_at`, then INSERT.
