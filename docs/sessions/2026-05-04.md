...
- Idempotent re-ingest landed:
  ...
  `content_hash` raises `ContentMismatchError` pending a future
  amendment-tracking decision.
- ADR-012 accepted:
...
- ADR-013 placeholder: amendment tracking / supersession policy tied to
  TODO-5 and `_skip_if_present()` mismatch behavior.
