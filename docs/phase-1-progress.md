...
> **Next session task**: decide the next executable boundary: rerun binary attachment retention for the rebuilt DB, draft the Rule `parent_doc_id` ADR-009 follow-up, or start the retrieval baseline.
...
3. **Recommended next action: choose the next executable boundary.** Options: rerun binary attachment retention for the rebuilt DB, draft the ADR-009 follow-up for Rule `parent_doc_id`, or start retrieval baseline work.
4. If binary attachment retention matters before retrieval, rerun the downloader because the DB rebuild reset `stored_file_path`, `checksum_sha256`, and `fetched_at` fields even though local files remain on disk.
...
- Default Compose DB rebuild — completed in the 2026-05-06 night session via `docker compose down -v`, `docker compose up -d database`, and canonical `eflaw` ingest. Re-run ingest skipped all 11 source rows by hash; `legal_documents.source_url` has zero non-`eflaw` rows.
...
- v1.19 (2026-05-06 night): Default Compose DB rebuilt from canonical `data/raw/eflaw`. Migrations `001`-`004` applied on a fresh `pgdata` volume; canonical ingest populated the same counts as the clean smoke DB; idempotent re-run skipped all 11 source rows by hash. Binary retention metadata is reset by design and needs a downloader pass if durable attachment paths are required before retrieval.
