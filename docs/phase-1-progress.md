...
> **Status**: Phase-1 statute ERD frozen (ADR-010, 2026-04-29), amended by ADR-013 (`is_current` → `is_head`) and ADR-014 (parallel `annex_attachments` / `form_attachments`; no `attachment_blobs` in Phase 1). ADR-015 accepted raw SQL migration management with a `schema_migrations` ledger. ADR-016 accepted annex attachment binary retention under `data/annexes/`; ADR-017 narrows default retention to PDF with HWP fallback; ADR-018 de-layouts annex `content_text` for retrieval. ADR-019 expands Phase 1 from SAPA-only to the SAPA + OSH full statutory families and makes the first retrieval baseline hybrid. OSH API discovery started and hit ADR-019's stop condition: `target=eflaw` returns distinct XML for the same `(law_id, mst)` at different `efYd` values. ADR-020 is drafted as `Proposed`; revised position is `target=eflaw` as the canonical statute XML source, with `target=law` auxiliary only. SAPA Act + Decree ingestion remains DB-verified. Running Compose `database`: 2 `legal_documents`, 240 normalized `structure_nodes`, 5 `annexes`, 21 `annex_attachments`; 5 PDF attachment rows are locally retained and HWP/image rows remain provenance-only (2026-05-06).
> **Generated**: Hand-off artifact for next session recall (now superseded by ADRs 001–019 for accepted Phase-1 statute schema, ingestion, and scope decisions; ADR-020 is proposed but not accepted).
> **Next session task**: review ADR-020. Do not write OSH raw files, migrations, parser changes, or ingestion rows until the effective-date raw identity decision is accepted.
...
3. **Recommended next action: review ADR-020.** The OSH API discovery confirmed distinct `target=eflaw` XML for the same `(law_id, mst)` at different `efYd` values. ADR-020 now proposes `target=eflaw` as canonical, `target=law` as auxiliary, `data/raw/eflaw/{law_id}/{mst}/{efYd}.xml`, and DB identity `(law_id, mst, effective_date)`.
4. **Do not write OSH source rows before ADR-020 acceptance.** The current `data/raw/{law_id}/{mst}.xml` path, legacy `target=law` retained XML, and `UNIQUE(mst)` invariant collapse staged effective-date payloads.
5. **After ADR-020 approval, complete source-ingestion gaps for OSH.** ADR-013 slice supersession behavior, persistence-only `supplementary_provisions`, and persistence-only `forms` / `form_attachments` are active blockers because OSH has 시행규칙 and forms.
...
- ADR-020 `eflaw` canonical statute XML source — drafted on 2026-05-06 with status `Proposed` after OSH discovery proved same-MST/different-efYd XML divergence; revised so `target=law` is auxiliary only. Not accepted yet.
