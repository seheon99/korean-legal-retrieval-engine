...
> **Status**: Phase-1 statute ERD frozen (ADR-010, 2026-04-29), amended by ADR-013 (`is_current` → `is_head`) and ADR-014 (parallel `annex_attachments` / `form_attachments`; no `attachment_blobs` in Phase 1). ADR-015 accepted raw SQL migration management with a `schema_migrations` ledger. Schema baseline committed at `migrations/001_statute_tables.sql`; first additive migration landed as `migrations/002_annex_form_attachments.sql`. ADR-009 population rule, idempotent re-ingest, `structure_nodes`, and `annexes` + `annex_attachments` ingestion are DB-verified. Running Compose `database`: 2 `legal_documents`, 240 normalized `structure_nodes`, 5 `annexes`, 21 `annex_attachments` (2026-05-05).
> **Generated**: Hand-off artifact for next session recall (now superseded by ADRs 001–015 for Phase-1 statute schema + ingestion decisions; this document remains canonical for cross-category context, license matrix, and DA log).
...
**Phase-1 statute ERD is frozen** as `migrations/001_statute_tables.sql` (ADR-010, 2026-04-29), with ADR-013 amending the temporality flag from `is_current` to `is_head` and ADR-014 adding parallel `annex_attachments` / `form_attachments` while deferring `attachment_blobs`. ADR-015 sets post-freeze migration management as ordered raw SQL files plus a `schema_migrations` ledger. Accepted decision context for the Phase-1 statute schema + ingestion lives in `docs/decisions/ADR-001` through `ADR-015`. The current phase is **ingestion-pipeline implementation**. `legal_documents`, `structure_nodes`, and `annexes` now load end-to-end for the Phase-1 Act + Decree corpus; the running Compose `database` service is filled with 2 documents, 240 normalized `structure_nodes`, 5 `annexes`, and 21 `annex_attachments`.
...
- Running dev DB fill — current counts after 2026-05-05 backfill: 2 `legal_documents`, 240 `structure_nodes`, 232 non-root `structure_nodes` with `parent_id`, 5 `annexes`, 21 `annex_attachments`.
