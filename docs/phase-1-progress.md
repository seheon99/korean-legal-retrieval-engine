...
> **Next session task**: implement ADR-020: add the `(law_id, mst, effective_date)` uniqueness migration, fetch canonical `eflaw` XML under `data/raw/eflaw/{law_id}/{mst}/{efYd}.xml`, and replace legacy SAPA raw XML after verified equivalent `eflaw` fetches.
...
3. **Recommended next action: implement ADR-020.** Add migration `004` replacing `UNIQUE(mst)` with uniqueness over `(law_id, mst, effective_date)`.
4. Fetch canonical `eflaw` XML to `data/raw/eflaw/{law_id}/{mst}/{efYd}.xml`. Replace or remove legacy SAPA `target=law` raw XML only after verified equivalent `eflaw` slices exist.
