...
3. **Recommended next action: review ADR-020.** The OSH API discovery confirmed distinct `target=eflaw` XML for the same `(law_id, mst)` at different `efYd` values. ADR-020 proposes `data/raw/eflaw/{law_id}/{mst}/{efYd}.xml` and DB identity `(law_id, mst, effective_date)`.
4. **Do not write OSH source rows before ADR-020 acceptance.** The current `data/raw/{law_id}/{mst}.xml` path and `UNIQUE(mst)` invariant collapse staged effective-date payloads.
...
- ADR-020 effective-law raw identity — drafted on 2026-05-06 with status `Proposed` after OSH discovery proved same-MST/different-efYd XML divergence. Not accepted yet.
